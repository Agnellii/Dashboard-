...
with col_graf2:
    if not df_filtrado.empty:
        ...
        grafico_hist.update_layout(title_x=.01)
        st.plotly_chart(grafico_hist, use_container_width=True)
    else:
        st.warning("Nenhum dado para exibir no gráfico de distribuição.")


col3_graf3, col4_graf4 = st.columns(2)
...
with col4_graf4:
    if not df_filtrado.empty:
        df_dm = df_filtrado[df_filtrado['cargo']== 'Data Scientist']
        media_do_pais = df_dm.groupby('residencia_iso3')['usd'].mean().reset_index()
